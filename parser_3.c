...
/*   Updated: 2022/11/25 18:15:14 by soubella         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_elements	*var_expr(t_parser *parser, bool var_expantion)
{
	...
	elements = elements_new_cap(1);
	if (var_expantion)
		elements_add(elements, current_lexeme(parser), VAR_ELEMENT, true);
	else
	{
		temp = string_create(string_join(dollar->lexeme,
					parser_consume(parser)->lexeme), true);
		elements_add(elements, temp, WORD_ELEMENT, false);
	}
	return (elements);
}
...
t_elements	*string_expr(t_parser *parser, bool var_expantion)
{
	...
	while (!parser_current_is(parser, CLOSE_DOUBLE_QUOTE))
	{
		...
		{
			temp = var_expr(parser, var_expantion);
